	...
	if( command === "bday" ) {
      if( message ) {
        ...
        try {
          ...
          if( month && day && username ) {
            ...
            if( monthNum >= 0 ) {
              ...
              store.put( "birthdays", userData );
              ComfyDiscord.Say( channel, "Saved Birthday: " + monthToString(monthNum) + " " + dayNum );
            }
          }
          else {
            if( monthNum >= 0 ) {
              // Print out the birthdays of the month
              var birthdayPeeps = getBirthdays( monthNum );
              ComfyDiscord.Say( channel, monthToString(monthNum) + " birthdays are: " + birthdayPeeps.map( x => `${x.name} (${monthToString(x.month)} ${x.day})` ).join( ", " ) );
            }
            else {
              // Check it as a username
              if( userData[ month ] ) {
                var birthday = userData[ month ];
                console.log( birthday );
                ComfyDiscord.Say( channel, birthday.name + "'s birthday is: " + monthToString( birthday.month ) + " " + birthday.day );
              }
            }
          }
        }
        ...
        // if( flags.broadcaster || flags.mod ) {
          // Print out the birthdays of the month
          var birthdayPeeps = getBirthdays( new Date().getMonth() );
          ComfyDiscord.Say( channel, "This month's birthdays are: " + birthdayPeeps.map( x => `${x.name} (${monthToString(x.month)} ${x.day})` ).join( ", " ) );
        // }
      }
    }
    else if( command === "horoscope" ) {
      ...
      if( userData[ username ] ) {
        ...
        let zodiac = await fetch( `http://localhost:1111/zodiac?month=${birthday.month + 1}&date=${birthday.day}` ).then( r => r.json() );
        if( zodiac.sign ) {
          let horoscope = await fetch( `http://localhost:1111/horoscope?fast=true&user=${username}&sign=${zodiac.sign}&noTwitch=true` ).then( r => r.json() );
	ComfyDiscord.Say( channel, `${username} - ${zodiac.sign}: ${horoscope.text}` );
        }
      }
      else {
        // Show username not found
        ComfyDiscord.Say( channel, `${username} was not found. Type !bday [month] [date] to add your birthday!` );
      }
    }
};
